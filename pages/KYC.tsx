...
export const KYCPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await BlockchainService.registerUser(updatedProfile, (hash) => {
                setTxHash(hash);
            });
            await refreshData();
        } catch (e: any) {
            console.error(e);
            if (e.message && e.message.includes("KYC already pending")) {
                setSubmitError("A KYC application is already pending for this wallet. Please wait for admin approval.");
            } else {
                setSubmitError("Failed to submit KYC. Please try again.");
            }
        } finally {
    ...
    if (isSubmitting && txHash) {
        return (
            <div className="max-w-2xl mx-auto pt-20 text-center">
                <div className="bg-[#151A23] p-12 rounded-2xl shadow-2xl border border-white/5 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-500 to-purple-500 animate-pulse"></div>
                    <div className="mx-auto h-24 w-24 rounded-full bg-indigo-500/10 flex items-center justify-center mb-6 text-indigo-400 animate-bounce">
                        <Upload size={48} />
                    </div>
                    <h2 className="text-2xl font-bold text-white mb-4">Transaction Submitted!</h2>
                    <p className="text-slate-400 mb-8">
                        Your KYC data has been broadcast to the blockchain. We are waiting for block confirmation.
                    </p>
                    <div className="bg-black/30 rounded-lg p-4 mb-8 font-mono text-xs text-slate-500 break-all border border-white/5">
                        {txHash}
                    </div>
                    <a
                    ...
                    >
                        View on Etherscan <span className="ml-2">→</span>
                    </a>
                </div>
            </div>
        );
    ...
    return (
        <div className="max-w-3xl mx-auto">
            ...
            {submitError && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 mb-6 flex items-center space-x-3 text-red-400">
                    <AlertCircle size={20} />
